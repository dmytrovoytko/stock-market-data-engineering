...
from settings import DATA_DIR, SCRAPE, START_YEAR, START_DATE, DEBUG
...
def extract_tickers_data(tickers, start_date=START_DATE, end_date=None, scrape=True):
    if DEBUG:
        print("extract_tickers_data:", tickers, start_date, end_date, scrape)
    t_start = time()
    if scrape:
        if end_date == None:
        ...
        for ticker in tickers:
            ...
            print(f" Downloaded {ticker}: {df_.shape[0]} record(s)")
            if df_.shape[0] == 0:
                failed_tickers.append(ticker)
            sleep(0.5)  # delay to prevent yf scraping errors "rate limit exceeded"
            # extra calculations
        ...
        if failed_tickers:
            ...
            print("All tickers downloaded normally.\n")
    ...
    print(f"Total selected: {df.shape[0]}/{records_read} record(s), took {(time() - t_start):.3f} second(s)")
    return df
...
def extract_data(mode=SAMPLE, selected_tickers=SELECTED_TICKERS, scrape=SCRAPE):
    ...
    end_date = yesterday  # Until market is closed we have no data for current date
    ...
    if DEBUG:
        print(" Latest date:", df_latest_date(df_tickers))
...
if __name__ == "__main__":
    ...
    # df_tickers_info, df_tickers = extract_data(mode=BIGQUERY, selected_tickers=selected_tickers, scrape=True)
    df_tickers_info, df_tickers = extract_data(mode=SAMPLE, selected_tickers=selected_tickers, scrape=True)  # True False
