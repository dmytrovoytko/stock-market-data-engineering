...
from settings import DATA_DIR, VISUALS_DIR, DEBUG
...
def duckdb_load_data(df, table_name, description, connection=""):
    if DEBUG:
        print(f"\nLoading: {description} {table_name} {connection}")
    ...
    try:
        ...
        print(f" Loaded. Total records in {table_name}:", res.fetchone()[0], f", took {(time() - t_start):.3f} second(s)")
    except Exception as e:
        print(f"! Error while loading {description} to duckdb {table_name}.\n{e}")
        return 1
...
def duckdb_transform_data(table_name, year=START_YEAR, description="", connection=""):
    if DEBUG:
        print(f"\nTransforming: {description} {table_name} {connection}")
    con, tables = duckdb_connect(connection)
    if not con:
        return 1
...
if __name__ == "__main__":
    if not (DUCKDB_CONNECTION):
        print(f"!! Check DUCKDB_CONNECTION settings. Connection failed.\n")
        exit(1)
